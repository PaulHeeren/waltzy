# -*- coding: utf-8 -*-
"""
Created on Wed Jul 26 17:18:46 2017

Utilities specific for the Waltzy project. Copied from cafeutils and modified
to our needs. Some work still to do (headers of fits-files etc.)...

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!MIGHT NOT NEED FUNCTION b_col (SEEMS TO BE ONLY THERE IN CAFEUTILS)!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!Function OverscanTrim???!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
#    d1 = b_col(d1)      #???????????????????????????????????????????????????
    ...
    if n>1:
        for i in range(n-1):
            ...
            if zero == 'none':
                d1 = np.dstack((d1,OverscanTrim(td[0].data)))
                
            else:
                d1 = np.dstack((d1,OverscanTrim(td[0].data)-z.data))
...
""" From cafepipe. We don't need that.
def b_col(d):
