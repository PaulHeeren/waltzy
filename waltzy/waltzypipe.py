# -*- coding: utf-8 -*-
"""
Created on Wed Jul 26 16:37:24 2017

DATA REDUCTION PIPELINE FOR PROJECT WALTZY

Based on the CERES pipeline by Brahm, Jordan and Espinoza, see:
Brahm et al. 2017, https://ui.adsabs.harvard.edu/#abs/2017PASP..129c4002B/abstract

Later to be extended for wavelength calibration with an iodine cell...
...
# use 'insert' to first search in this path for jplephem
# (otherwise dedicated Python-module might be imported)
sys.path.insert(1, baryc_dir)
ephemeris='DEc403'
...
""" This is how it should be like in the end: classify all images by information
    from their headers. Not possible yet.
    
# collect all raw data
biases, flats, ThAr_ref, sim_sci, ThAr_ref_dates, obnames, exptimes = waltzyutils.FileClassify(dirin,log)
...
new_list = np.array([dirin+"arcturus_300s_shift.fits"])

# Plus generating combined flat from cont+led
n = len(Flat_ref)
...
if (pre_process == 1):
    """ Also no bias frames yet
    # median combine Biases
    print "\tGenerating Master calibration frames..."
    MasterBias, RO_bias, GA_bias = waltzyutils.MedianCombine(biases)
    ...
    Flat, RO_fl, GA_fl = waltzyutils.MedianCombine(flats, zero=dirout+'MasterBias.fits')
...
"""
    ...
    """ p2 to be able to check the traces. Need to hand it in GLOBALutils then. I changed one version
    of that to be able to do it."""
    c_all, nord, p2 = GLOBALutils.get_them(Flat, ext_aperture, trace_degree, maxords=-1, nsigmas=10., mode=1)
    ...
    # recover trace coefficients and order number
...
if ( os.access(P_fits,os.F_OK) == False ) or \
   ...
   (force_flat_extract):
    ...
    bac = GLOBALutils.get_scat(Flat,Centers,span=9)
    fl = Flat - bac
    plot(fl[:,1000])
...
if (os.access(S_flat_n_fits,os.F_OK)):
    os.remove( S_flat_n_fits )
hdu = pyfits.PrimaryHDU( S_flat_n )
hdu.writeto( S_flat_n_fits )

# From cafepipe: Sort the science spectra by date.
# Then use only those ThAr spectra that lie within this time frame.
"""
new_list = []
new_times = []
for fsim in sim_sci:
    ...
    if object2do in h[0].header['OBJECT'] or object2do == 'all':
        ...
        mjd, mjd0 = waltzyutils.mjd_fromheader2( h )
...
for fsim in ThAr_ref:
    ...
    if ( os.access(thar_fits,os.F_OK) == False ) or ( force_thar_extract ):
        ...
        #No bias frames yet#################################################
        #dthar = waltzyutils.OverscanTrim( hthar[0].data ) - MasterBias
        dthar = hthar[0].data
        ...
        # no tron/tgain yet ####################################################
        #tron, tgain = hthar[0].header['CCDRON'], hthar[0].header['CCDGAIN']
        tron, tgain = 50, 4
        #print tR,tG
...
for i in range(len(ThAr_ref)):
    ...
    #tgain, tron = float(hd['CCDGAIN']), float(hd['CCDRON'])
    tron, tgain = 50, 4
    ...
    if ( os.access(wavsol_pkl,os.F_OK) == False ) or (force_thar_wavcal):
        ...
        # no mjd information yet ###########################################        
        #hthar = pyfits.open( ThAr_ref[i] )
        #mjd, mjd0 = waltzyutils.mjd_fromheader2( hthar )
        mjd = 2000.0
        thar_fits = dirout + ThAr_ref[i].split('/')[-1][:-4]+'spec.fits.S'
        thar_S = pyfits.getdata( thar_fits )
        lines_thar  = thar_S[:,1,:]
        # Error calculation: Either use errors from above (see next line)
        ...
        for order in range(ro0,ro0+n_useful,1):
            ...
            # Think about putting in wei for np.ones(len(thar_order)); originally minlines=10
        ...
        # Think about putting in 200 in maxrms for MRMS; originally minlines=200
...
# This is from cafepipe: To determine instrumental drift during the night (with respect
# to ThAr-spec 0).
# Maybe don't need it?!
...
p_ref = dct['p1']
#mjds_thar,shifts = [],[]#################################################
mjds_thar,shifts = [57914.8955, 57914.896],[]
...
for i in range(len(ThAr_ref)):
    ...
    p_shift, pix_centers, orders, wavelengths, I, rms_ms, residuals = \
                ...
                dct['All_Wavelengths'], dct['All_Orders'], np.ones(len(dct['All_Orders'])), \
                p_ref, order0=ro0, npix=npix, Cheby=use_cheby, ntotal=n_useful, maxrms=200, \
                Inv=Inverse_m, minlines=500, nx=ncoef_x, nm=ncoef_m)
...
for fsim in new_list:
    ...
    # Object name
    #obname    = h[0].header['OBJECT'] ################################# not there yet
    obname = "Arcturus"
    ...
    if ( os.access(dirout+fout ,os.F_OK) == False ) or (force_spectral_file_build):
        ...
        hdu = GLOBALutils.update_header(hdu,'HIERARCH SHUTTER START DATE', 1234)#h[0].header['DATE'][:10] )
        hdu = GLOBALutils.update_header(hdu,'HIERARCH SHUTTER START UT',  12345)#h[0].header['DATE'][11:])
        hdu = GLOBALutils.update_header(hdu,'HIERARCH TEXP (S)', 300)#h[0].header['EXPTIME'])
        ...
        hdu = GLOBALutils.update_header(hdu,'HIERARCH RA', ra)#h[0].header['RA'])
        hdu = GLOBALutils.update_header(hdu,'HIERARCH DEC', dec)#h[0].header['DEC'])
        hdu = GLOBALutils.update_header(hdu,'HIERARCH RA BARY',ra)
        hdu = GLOBALutils.update_header(hdu,'HIERARCH DEC BARY',dec)
        hdu = GLOBALutils.update_header(hdu,'HIERARCH EQUINOX', epoch)#h[0].header['EQUINOX'])
        hdu = GLOBALutils.update_header(hdu,'HIERARCH OBS LATITUDE', latitude)#h[0].header['HIERARCH CAHA TEL GEOLAT'])
        hdu = GLOBALutils.update_header(hdu,'HIERARCH OBS LONGITUDE', longitude)#h[0].header['HIERARCH CAHA TEL GEOLON'])
        hdu = GLOBALutils.update_header(hdu,'HIERARCH OBS ALTITUDE', altitude)#h[0].header['HIERARCH CAHA TEL GEOELEV'])
        hdu = GLOBALutils.update_header(hdu,'HIERARCH TARG AIRMASS START', 1.5)#h[0].header['AIRMASS'])
        ...
        if (not JustExtract):
            ...
            moon_dict = {'moonmatters':moonmatters,'moon_state':moon_state,'moonsep':moonsep,\
		         'lunation':lunation,'mephem':mephem,'texp': 300}
           #float(h[0].header['EXPTIME'])}###############################################
            ...
            SNR_5130 = np.median(spec[8,105 - difo,900:1101] )
            airmass  = 1.5 #float(h[0].header['AIRMASS']) ################################
            seeing   = 2.5

            TEXP = 300 #float(h[0].header['EXPTIME'])####################################
            ...
            # write to output
            line_out = "%-15s %18.8f %9.4f %7.4f %9.3f %5.3f   waltzy   ceres   65000 %6d %5.2f %5.2f %5.1f %4.2f %5.2f %6.1f %4d %s\n"%\
